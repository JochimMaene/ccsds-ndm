...
this command in a separate clean-checkout job and will not publish unless it succeeds. It also
requires the release tag to equal `v` followed by the version in `Cargo.toml`.
