...
| Stable structured diagnostics | Partial | Every safely reachable OPM missing-required-field, invalid-choice, invalid-value, and out-of-range generation failure exposes a stable code and complete model path relative to the message root through `ValidationError::code` and `ValidationError::field_path`. Focused mutations cover required fields, both invalid Keplerian anomaly selections, XML text positions, every public numeric block, maneuver duration units, all five spacecraft ranges, all seven Keplerian ranges, and both maneuver ranges. The anomaly-choice path identifies its containing block rather than inventing a singular model field. Empty required epochs and invalid epoch text are unrepresentable through the public `CalendarEpoch` type. `every_opm_3_xml_generation_entry_point_rejects_an_invalid_model` compatibility-tests one diagnostic across every Rust XML-generation entry point. Path context and choice details allocate only when validation fails. Unsupported-version, serialization, and output-error categories are not yet stabilized. |
...
Define the smallest top-level diagnostic-code API needed by non-validation generation failures
before stabilizing unsupported-output-version and writer errors. Do not force a model field path
onto failures that do not concern a model field. Do not mark the capability `verified` until every
remaining `Partial` and `Gap` above is closed.
