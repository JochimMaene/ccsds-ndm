...
| Representative performance | Covered | `opm_xml_allocations` enforces materialized, streaming, and configured-limit allocation budgets, and the streaming implementation writes directly to the caller sink without retaining a document buffer. Wall-clock benchmarking is not registered for OPM, which generates in microseconds; allocation budgets are the deterministic gate. |
