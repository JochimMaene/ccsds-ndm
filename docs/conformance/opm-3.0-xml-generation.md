...
- [focused OPM 3.0 XML-generation tests](../../ccsds-ndm/tests/opm/generation.rs);
- [Keplerian mutation and boundary tests](../../ccsds-ndm/tests/opm/validation.rs);
- [maneuver duration-unit tests](../../ccsds-ndm/tests/opm/validation.rs); and
- [shared regression tests](../../ccsds-ndm/tests/review_feedback_regressions.rs);
...
| 3.2.4 Keplerian block | The block is absent or complete and contains exactly one of true or mean anomaly. | Covered | Non-optional fields enforce block completeness. `KeplerianElements::validate` checks every public numeric value against the XSD facets and enforces the anomaly choice; [focused mutation and boundary tests](../../ccsds-ndm/tests/opm/validation.rs) exercise the rules. |
...
| Public Rust API contract | Covered | Functional tests exercise typed, version-aware, streaming, type-erased, and file entry points, including output and failure behavior. The raw serde XML serializer is crate-internal, so it cannot bypass the public generation gate. |
| Stable structured diagnostics | Covered | Every safely reachable OPM missing-required-field, invalid-choice, invalid-value, and out-of-range generation failure exposes a stable code and complete model path relative to the message root. The minimal diagnostic contract adds failure-only operation, notation, message kind, edition, code, optional path, and location context. Sink and file failures retain `io.error` without a model path. |
| Panic-free streaming output | Covered | [Failing-writer tests](../../ccsds-ndm/tests/message_output_contract.rs) cover multiple sink-failure boundaries and verify stable I/O diagnostics without panics. |
| Representative performance | Covered | `xml_message_matrix/generate/opm` measures materialized XML generation of the shipped OPM fixture, including the shared validation gate in each iteration, and reports throughput using generated output bytes. `opm_xml_allocations` enforces materialized and streaming allocation budgets, and the streaming implementation writes directly to the caller sink without retaining a document buffer. Wall-clock results are informational under the proportionate pre-1.0 policy. |
| Rust surface/release gates | Covered | CI tests Rust 1.88 as the MSRV. `just package-rust` packages, extracts, and tests the artifact; tag releases require the tag to match the crate version. |
