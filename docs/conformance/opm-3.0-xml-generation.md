...
| Stable structured diagnostics | Covered | Every safely reachable OPM missing-required-field, invalid-choice, invalid-value, and out-of-range generation failure exposes a stable code and complete model path relative to the message root. The [minimal diagnostic contract](../design/opm-generation-diagnostics.md) adds failure-only operation, notation, message kind, edition, code, optional path, and location context. Sink and file failures retain `io.error` without a model path. |
...
| Rust surface/release gates | Covered | The [pre-1.0 Rust release policy](../rust-release-policy.md) declares and tests Rust 1.88 as the MSRV and pins development to Rust 1.90.0 on Ubuntu, records the unstable compatibility contract and public-only security-reporting scope, and does not require byte-identical artifacts before 1.0. `just package-rust` packages, extracts, and tests the artifact; clean-checkout CI also requires the tag to match the crate version. |
