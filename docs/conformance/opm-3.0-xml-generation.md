...
| Rust surface/release gates | Partial | The [pre-1.0 Rust release policy](../rust-release-policy.md) records the tested stable-Rust/Ubuntu scope, explicitly unstable compatibility contract, migration expectation, and current security-reporting limitation. `just package-rust` and the clean-checkout CI packaging job exercise Cargo's package selection and verify the unpacked build before publishing; the release job also requires the tag to match the crate version. MSRV, additional platforms, private security reporting, artifact installation, and byte-reproducible release evidence remain open. |
