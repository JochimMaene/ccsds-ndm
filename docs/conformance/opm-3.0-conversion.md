...
| --- | --- | --- |
| Both directions and edition preservation | Covered | `opm::conversion::both_conversion_directions_preserve_the_complete_typed_model` covers every shipped OPM KVN fixture and the standalone XML fixture. Generation preserves the parsed edition. |
...
| String, file, and dispatch surfaces | Covered | `opm::conversion` covers string conversion; `message_output_contract` covers typed/type-erased output and atomic file replacement for AEM, OEM, and OPM alike. Parse/generation diagnostic suites cover invalid source and sink failures. |
| Failure behavior | Covered | Failed file conversion preserves the existing destination and leaves no temporary file. |
...
`opm::conversion::kvn_merges_data_and_state_vector_comments_into_the_data_section` fixes this
contract.
