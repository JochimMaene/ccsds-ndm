...
| Reproducible workloads | Existing `ocm_trajectory_10k` (CARTPV), `ocm_covariance_10k` (CARTP) and `ocm_maneuver_10k` Criterion groups measure KVN and XML parse and generation through one shared registration helper. The shared family matrices add representative small-message comparisons. |
