...
| Values | ODM 5.2, 7.5, 8.13 | Required fields must be present. In KVN, the time systems and frames the book lists must be all upper or all lower case. XML numbers follow `xsd:double`, so `INF`, `-INF` and `NaN` are allowed. KVN numbers must be finite and fit in a double; reading accepts more than 16 digits. Units are fixed and filled in when missing. | `oem::model`, `oem::validation`, `oem::conversion` |
...
- The object must be the same in every segment (5.1.3). `OBJECT_NAME` and `OBJECT_ID` are
  ...
  `Mars Global Surveyor` and `MARS_GLOBAL_SURVEYOR` are one object. The time system comparison (5.2.4.5) also ignores case and, in XML, surrounding blanks (` UTC ` names UTC); the text is kept as written.
