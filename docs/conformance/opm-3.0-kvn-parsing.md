...
| Character repertoire, line endings, and 254-character line bound | Covered | `opm::parsing` rejects non-ASCII/control data and overlong lines; all four normative terminators (CR, LF, CRLF, and LFCR) parse to equal models. |
...
| Located bounded diagnostics | Covered | `opm::parsing` fixes operation/notation/message/edition/code, byte/line/column, a 128-character token excerpt, expected context, and field paths. |
| Resource behavior | Covered | `opm::parsing` covers optional exact aggregate byte limits and stable resource codes; the normative line bound applies without configuration. |
| Performance and fuzzing | Covered | OPM carries no record history and is not registered in the benchmark suite; it parses in microseconds, and the shared KVN parsing code is measured by the history-carrying families. The generic KVN fuzz target reaches OPM from a checked-in minimal OPM seed, and the reproducible smoke command completes without a crash. Wall-clock thresholds and sustained fuzzing are optional discovery/maturity work, not deterministic pre-1.0 gates. |
