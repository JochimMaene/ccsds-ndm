...
| Representative performance | Covered | `opm_kvn_allocations` enforces deterministic allocation budgets for materialized and streaming generation of the richest Annex G numeric/covariance fixture. Wall-clock benchmarking is not registered for OPM, which generates in microseconds. Wall-clock results are informational under the proportionate pre-1.0 policy. |
