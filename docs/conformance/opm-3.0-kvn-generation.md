...
| Diagnostics, resource behavior, and performance | Partial | `just bench-opm-kvn` reproducibly measures materialized and streaming generation of the richest Annex G numeric/covariance fixture. The numeric formatter uses a stack-resident `zmij` buffer and writes directly into the destination or a reusable streaming line buffer, without a per-value `String`. KVN-specific diagnostic evidence covers printable text, comment and assignment line limits, dynamic user-key attribution, keyword syntax, GM unit spelling, every optional numeric container, public pre-write rejection, and sink failures before, during, and after numeric/user-defined lines. Shared model diagnostics remain linked to the exhaustive XML-generation mutations. Allocation/resource measurements, repeated-maneuver scaling, and a published regression budget remain open. |

## Next Small Implementation Step

Add repeated-maneuver scaling to the existing OPM KVN benchmark without introducing a separate
benchmark framework. Use that evidence to decide whether a concrete regression budget is justified.
