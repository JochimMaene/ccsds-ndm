...
| Diagnostics, resource behavior, and performance | Partial | `just bench-opm-kvn` reproducibly measures materialized and streaming generation of the richest Annex G numeric/covariance fixture. The numeric formatter uses a stack-resident `zmij` buffer and writes directly into the destination or a reusable streaming line buffer, without a per-value `String`. Focused sink failures before, during, and after numeric/user-defined lines demonstrate panic-free propagation of the original I/O error with stable `io.error` diagnostics. Complete diagnostic mutations, allocation/resource measurements, repeated-maneuver scaling, and a published regression budget remain open. |

## Next Small Implementation Step

Inventory the remaining reachable OPM KVN generation diagnostics and add table-driven mutations
only where the existing XML-generation evidence does not already exercise the shared model gate.
