...
| Diagnostics and resource behavior | Covered | The [minimal diagnostic contract](../design/opm-generation-diagnostics.md) and `opm_generation_diagnostics` cover identical structured context across all public entry points. `opm_generation_limits` covers the optional aggregate bound, exact-bound success, stable resource diagnostics, and zero-byte streaming rejection. The numeric formatter uses a stack-resident `zmij` buffer and writes directly into the destination or a reusable line buffer without a per-value `String`; `opm_kvn_allocations` enforces materialized and pre-sized streaming allocation budgets. Per-field limits are intentionally omitted because the aggregate bound plus the normative 254-character line limit cover the demonstrated need. |
| Representative performance | Partial | `just bench-opm-kvn` reproducibly measures materialized and streaming generation of the richest Annex G numeric/covariance fixture, plus materialized generation with 1, 10, 100, and 1000 repeated maneuvers. The scaling benchmark keeps parsing and model construction outside the measured loop and the repository's CodSpeed simulation job runs these benchmarks on pull requests and `main`. A published noise-aware timing threshold remains open. |

## Next Small Implementation Step

Collect enough history for the new OPM workloads in the existing CodSpeed job to choose a
noise-aware timing regression budget. Local Criterion runs remain diagnostic and do not justify a
stable threshold.
