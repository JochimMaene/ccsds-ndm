...
| Deterministic output and public API contract | Covered | Functional tests cover the typed, version-aware, type-erased, streaming, and file entry points. The richest Annex G fixture produces identical bytes through each entry point. |
| Diagnostics and resource behavior | Covered | The minimal diagnostic contract and shared regression tests cover structured context and pre-write validation. The numeric formatter uses a stack-resident `zmij` buffer and writes directly into the destination or a reusable line buffer without a per-value `String`; `opm_kvn_allocations` enforces materialized and pre-sized streaming allocation budgets. |
| Representative performance | Covered | `cargo bench -p ccsds-ndm --bench kvn_benches -- 'kvn_message_matrix/generate/opm'` reproducibly measures materialized generation of the richest Annex G numeric/covariance fixture, keeping parsing and model construction outside the measured loop. `opm_kvn_allocations` enforces deterministic allocation budgets for both the materialized and the streaming path. Wall-clock results are informational under the proportionate pre-1.0 policy. |
