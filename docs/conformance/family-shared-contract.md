...
Status: implemented shared plumbing, not blanket CCSDS conformance. The verified OPM 3.0 and OEM
3.0 Rust cells remain governed by their message-specific inventories.
...
| Validated generation | String, streaming, Python, conversion, and CLI paths delegate to the Rust `Ndm` / `VersionedNdm` boundary. | Complete message-specific semantic and notation inventories, invalid-model tests, KVN golden ordering, and official-XSD generation evidence. |
...
| Rust/Python/CLI consistency | `family_contract`, `test_parse_and_generation_options.py`, and `opm_cli` exercise generic dispatch and shared limits without per-message adapter semantics. | Add exact capability-cell surface evidence and packaged-artifact checks before advertising parity. |
