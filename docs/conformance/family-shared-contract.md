...
| Rust/Python consistency | `family_contract` and `test_parse_and_generation_options.py` exercise generic dispatch without per-message adapter semantics. | Add exact capability-cell surface evidence and packaged-artifact checks before advertising parity. |
