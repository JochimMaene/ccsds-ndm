...
| Reproducible performance workload | `kvn_message_matrix` covers KVN parse/generate for the five history-carrying families (OEM, OCM, TDM, AEM, ACM); `xml_message_matrix` covers XML parse/generate for those five plus combined NDM. Flat families are not registered, because they complete in microseconds and share the same parsing and generation code. `just bench-family` reproduces both, and the existing CodSpeed workflow executes the benchmark targets. | Record reviewed baselines and add targeted allocation/scaling gates only for measured hotspots. Wall-clock thresholds remain informational. |
