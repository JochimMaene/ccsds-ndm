...
impl OcmCovarianceMatrix {
    fn validate(&self) -> Result<()> {
        crate::validation::epoch_precision(&[("COV_FRAME_EPOCH", &self.cov_frame_epoch)])?;
...
fn validate_ocm_metadata(metadata: &OcmMetadata) -> Result<()> {
    crate::validation::epoch_precision(&[
        ("EPOCH_TZERO", &metadata.epoch_tzero),
        ("PREVIOUS_MESSAGE_EPOCH", &metadata.previous_message_epoch),
        ("NEXT_MESSAGE_EPOCH", &metadata.next_message_epoch),
        ("START_TIME", &metadata.start_time),
        ("STOP_TIME", &metadata.stop_time),
        ("NEXT_LEAP_EPOCH", &metadata.next_leap_epoch),
    ])?;
...
impl OcmPerturbations {
    fn validate(&self) -> Result<()> {
        crate::validation::epoch_precision(&[("SW_DATA_EPOCH", &self.sw_data_epoch)])?;
...
impl OcmOdParameters {
    fn validate(&self) -> Result<()> {
        crate::validation::epoch_precision(&[("OD_EPOCH", &self.od_epoch)])?;
        for (name, val) in [
