...
pub fn ocm_metadata(input: &mut &str) -> KvnResult<OcmMetadata> {
    ...
    parse_block!(input, comment, {
        "OBJECT_NAME" => val: kv_string_opt => { object_name = val; },
        "INTERNATIONAL_DESIGNATOR" => val: kv_string_opt => { international_designator = val; },
        "CATALOG_NAME" => val: kv_string_opt => { catalog_name = val; },
        "OBJECT_DESIGNATOR" => val: kv_string_opt => { object_designator = val; },
        "ALTERNATE_NAMES" => val: kv_string_opt => { alternate_names = val; },
        "ORIGINATOR_POC" => val: kv_string_opt => { originator_poc = val; },
        "ORIGINATOR_POSITION" => val: kv_string_opt => { originator_position = val; },
        "ORIGINATOR_PHONE" => val: kv_string_opt => { originator_phone = val; },
        "ORIGINATOR_EMAIL" => val: kv_string_opt => { originator_email = val; },
        "ORIGINATOR_ADDRESS" => val: kv_string_opt => { originator_address = val; },
        "TECH_ORG" => val: kv_string_opt => { tech_org = val; },
        "TECH_POC" => val: kv_string_opt => { tech_poc = val; },
        "TECH_POSITION" => val: kv_string_opt => { tech_position = val; },
        "TECH_PHONE" => val: kv_string_opt => { tech_phone = val; },
        "TECH_EMAIL" => val: kv_string_opt => { tech_email = val; },
        "TECH_ADDRESS" => val: kv_string_opt => { tech_address = val; },
        "PREVIOUS_MESSAGE_ID" => val: kv_string_opt => { previous_message_id = val; },
        "NEXT_MESSAGE_ID" => val: kv_string_opt => { next_message_id = val; },
        "ADM_MSG_LINK" => val: kv_string_opt => { adm_msg_link = val; },
        "CDM_MSG_LINK" => val: kv_string_opt => { cdm_msg_link = val; },
        "PRM_MSG_LINK" => val: kv_string_opt => { prm_msg_link = val; },
        "RDM_MSG_LINK" => val: kv_string_opt => { rdm_msg_link = val; },
        "TDM_MSG_LINK" => val: kv_string_opt => { tdm_msg_link = val; },
        "OPERATOR" => val: kv_string_opt => { operator = val; },
        "OWNER" => val: kv_string_opt => { owner = val; },
        "COUNTRY" => val: kv_string_opt => { country = val; },
        "CONSTELLATION" => val: kv_string_opt => { constellation = val; },
        "OBJECT_TYPE" => object_type: kv_enum,
        "TIME_SYSTEM" => time_system: kv_string,
        "EPOCH_TZERO" => epoch_tzero: kv_epoch,
        "OPS_STATUS" => val: kv_string_opt => { ops_status = val; },
        "ORBIT_CATEGORY" => val: kv_string_opt => { orbit_category = val; },
        "OCM_DATA_ELEMENTS" => val: kv_string_opt => { ocm_data_elements = val; },
        ...
        "EOP_SOURCE" => val: kv_string_opt => { eop_source = val; },
        "INTERP_METHOD_EOP" => val: kv_string_opt => { interp_method_eop = val; },
        "CELESTIAL_SOURCE" => val: kv_string_opt => { celestial_source = val; },
...
pub fn ocm_traj_state(input: &mut &str) -> KvnResult<OcmTrajState> {
    ...
    parse_block!(input, comment, {
        "TRAJ_ID" => val: kv_string_opt => { traj_id = val; },
        "TRAJ_PREV_ID" => val: kv_string_opt => { traj_prev_id = val; },
        "TRAJ_NEXT_ID" => val: kv_string_opt => { traj_next_id = val; },
        "TRAJ_BASIS" => traj_basis: kv_enum,
        "TRAJ_BASIS_ID" => val: kv_string_opt => { traj_basis_id = val; },
        "INTERPOLATION" => val: kv_string_opt => { interpolation = val; },
        "INTERPOLATION_DEGREE" => interpolation_degree: kv_u32,
        "PROPAGATOR" => val: kv_string_opt => { propagator = val; },
        "CENTER_NAME" => val: kv_string_opt => { center_name = val; },
        "TRAJ_REF_FRAME" => val: kv_string_opt => { traj_ref_frame = val; },
        ...
        "ORB_AVERAGING" => val: kv_string_opt => { orb_averaging = val; },
        "TRAJ_UNITS" => val: kv_string_opt => { traj_units = val; },
...
pub fn ocm_phys(input: &mut &str) -> KvnResult<OcmPhysicalDescription> {
    ...
    parse_block!(input, comment, {
        "MANUFACTURER" => val: kv_string_opt => { manufacturer = val; },
        "BUS_MODEL" => val: kv_string_opt => { bus_model = val; },
        "DOCKED_WITH" => val: kv_string_opt => { docked_with = val; },
        ...
        "OEB_PARENT_FRAME" => val: kv_string_opt => { oeb_parent_frame = val; },
        ...
        "ATT_CONTROL_MODE" => val: kv_string_opt => { att_control_mode = val; },
        "ATT_ACTUATOR_TYPE" => val: kv_string_opt => { att_actuator_type = val; },
...
pub fn ocm_cov(input: &mut &str) -> KvnResult<OcmCovarianceMatrix> {
    ...
    parse_block!(input, comment, {
        "COV_ID" => val: kv_string_opt => { cov_id = val; },
        "COV_PREV_ID" => val: kv_string_opt => { cov_prev_id = val; },
        "COV_NEXT_ID" => val: kv_string_opt => { cov_next_id = val; },
        "COV_BASIS" => cov_basis: kv_enum,
        "COV_BASIS_ID" => val: kv_string_opt => { cov_basis_id = val; },
        "COV_REF_FRAME" => val: kv_string_opt => { cov_ref_frame = val; },
        "COV_FRAME_EPOCH" => cov_frame_epoch: kv_epoch,
        "COV_TYPE" => cov_type: kv_string,
        "COV_UNITS" => val: kv_string_opt => { cov_units = val; },
...
pub fn ocm_man(input: &mut &str) -> KvnResult<OcmManeuverParameters> {
    ...
    parse_block!(input, comment, {
        ...
        "MAN_PREV_ID" => val: kv_string_opt => { man_prev_id = val; },
        "MAN_NEXT_ID" => val: kv_string_opt => { man_next_id = val; },
        "MAN_BASIS" => man_basis: kv_enum,
        "MAN_BASIS_ID" => val: kv_string_opt => { man_basis_id = val; },
        "MAN_DEVICE_ID" => man_device_id: kv_string,
        "MAN_PREV_EPOCH" => man_prev_epoch: kv_epoch,
        "MAN_NEXT_EPOCH" => man_next_epoch: kv_epoch,
        "MAN_PURPOSE" => val: kv_string_opt => { man_purpose = val; },
        "MAN_PRED_SOURCE" => val: kv_string_opt => { man_pred_source = val; },
        "MAN_REF_FRAME" => val: kv_string_opt => { man_ref_frame = val; },
        "MAN_FRAME_EPOCH" => man_frame_epoch: kv_epoch,
        "GRAV_ASSIST_NAME" => val: kv_string_opt => { grav_assist_name = val; },
        ...
        "DC_BODY_FRAME" => val: kv_string_opt => { dc_body_frame = val; },
        ...
        "MAN_UNITS" => val: kv_string_opt => { man_units = val; },
...
pub fn ocm_pert(input: &mut &str) -> KvnResult<OcmPerturbations> {
    ...
    parse_block!(input, comment, {
        ...
        "OCEAN_TIDES_MODEL" => val: kv_string_opt => { ocean_tides_model = val; },
        "SOLID_TIDES_MODEL" => val: kv_string_opt => { solid_tides_model = val; },
        "REDUCTION_THEORY" => val: kv_string_opt => { reduction_theory = val; },
        "ALBEDO_MODEL" => val: kv_string_opt => { albedo_model = val; },
        "ALBEDO_GRID_SIZE" => albedo_grid_size: kv_u64,
        "SHADOW_MODEL" => val: kv_string_opt => { shadow_model = val; },
        "SHADOW_BODIES" => val: kv_string_opt => { shadow_bodies = val; },
        "SRP_MODEL" => val: kv_string_opt => { srp_model = val; },
        "SW_DATA_SOURCE" => val: kv_string_opt => { sw_data_source = val; },
        "SW_DATA_EPOCH" => sw_data_epoch: kv_epoch,
        "SW_INTERP_METHOD" => val: kv_string_opt => { sw_interp_method = val; },
...
pub fn ocm_od(input: &mut &str) -> KvnResult<OcmOdParameters> {
    ...
    parse_block!(input, comment, {
        ...
        "OD_PREV_ID" => val: kv_string_opt => { od_prev_id = val; },
        "OD_METHOD" => od_method: kv_string,
        "OD_EPOCH" => od_epoch: kv_epoch,
        "OD_TIME_TAG" => od_epoch: kv_epoch,
        "MAX_RESI_ACCEPTED" => val: kv_string_opt => { _max_resi_accepted = val; },
        ...
        "SOLVE_STATES" => val: kv_string_opt => { solve_states = val; },
        "CONSIDER_N" => consider_n: kv_u64,
        "CONSIDER_PARAMS" => val: kv_string_opt => { consider_params = val; },
        "SEDR" => sedr: kv_from_kvn,
        "SENSORS_N" => sensors_n: kv_u64,
        "SENSORS" => val: kv_string_opt => { sensors = val; },
        "WEIGHTED_RMS" => weighted_rms: kv_from_kvn,
        "DATA_TYPES" => val: kv_string_opt => { data_types = val; },
